...
export const workScheduleSchema = z
  ...
  .refine(
    (data) => {
      ...
      const dayOffCount = data.weeklySchedule.filter((day) => day.isDayOff).length;
  ...
  .refine(
    (data) => {
      ...
      if (data.type === 'time') {
        ...
        const workingDays = data.weeklySchedule.filter((day) => !day.isDayOff).length;
  ...
  .refine(
    (data) => {
      ...
      if (data.type === 'duration') {
        return data.duration !== undefined && data.duration >= 30 && data.duration <= 48;
      }
      return true;
    },
    {
      message: 'Weekly hours must be between 30 and 48 for duration-based schedules',
      path: ['duration'],
    },
  );
